...
class HParams:
	def __init__(
		...
		self.normalize_for_wavenet = normalize_for_wavenet
		self.clip_for_wavenet = clip_for_wavenet
...
def hparams_debug_string():
	#
	attrs = dir(hparams)[26:]
	hp = ["  %s: %s" % (name, hparams.__getattribute__(name)) for name in sorted(attrs) if name != "sentences"]
